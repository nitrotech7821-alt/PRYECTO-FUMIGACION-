...
def componentes_ubicacion():
    """Dos botones: toma el GPS real del dispositivo y abre Google Maps en ese punto."""
    ...
    if gps_lat and gps_lon:
        maps_url = f"https://www.google.com/maps/@?api=1&map_action=map&center={gps_lat},{gps_lon}&zoom=18"
    else:
        maps_url = ""
    ...
    <div style="font-family:Arial,sans-serif;width:100%;box-sizing:border-box;">
      ...
      <div id="gpsMsg" style="margin-top:9px;min-height:18px;font-size:14px;color:#666;text-align:center;"></div>

      <a id="mapBtn" href="{maps_url}" target="_blank" rel="noopener noreferrer" style="
        display:block;width:100%;height:58px;line-height:58px;text-align:center;
        box-sizing:border-box;padding:0 16px;border-radius:10px;
        background:#303030;color:#fff;font-weight:800;font-size:18px;
        text-decoration:none;box-shadow:0 3px 7px rgba(0,0,0,.18);margin-top:10px;">
        ABRIR MAPA &nbsp;🗺️
      </a>
        ...
        btn.onclick = function() {{
          ...
          navigator.geolocation.getCurrentPosition(
            function(pos) {{
              ...
              // Guarda las coordenadas en la URL de la aplicación para que
              // Streamlit las recupere y las conserve en session_state.
              const url = new URL(window.parent.location.href);
              url.searchParams.set('gps_lat', la);
              url.searchParams.set('gps_lon', lo);
            ...
            function(err) {{
              ...
              if (err.code === 1) t = '❌ Permiso denegado. Permite la ubicación en el celular.';
              if (err.code === 2) t = '❌ Ubicación no disponible. Revisa el GPS.';
              if (err.code === 3) t = '❌ Se agotó el tiempo. Intenta nuevamente.';
              msg.innerHTML = t;
            }},
