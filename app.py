...
with st.container(border=True):
    ...
    if current_lat is not None and current_lon is not None:
        ...
        delta = 0.006
        left, right = current_lon-delta, current_lon+delta
        bottom, top = current_lat-delta, current_lat+delta
        osm_url = (
        ...
        components.html(f"""
        <iframe src="{osm_url}" width="100%" height="420"
          style="border:1px solid #d0d0d0;border-radius:10px;" loading="lazy"></iframe>
        """, height=440)

        st.link_button("🗺️ ABRIR ESTA UBICACIÓN EN GOOGLE MAPS",
                       mapa_url, use_container_width=True)
    else:
        st.info("📍 Presiona el botón de ubicación y permite el acceso al GPS.")
