...
def save_record(data):
    with sqlite3.connect(DB_PATH) as con:
        con.execute('''INSERT INTO inspecciones
            (created_at, inspector, folio, domicilio, colonia, telefono, observaciones,
             respuestas_json, entorno_json, foto_path, total_puntos)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (data['created_at'], data['inspector'], data['folio'], data['domicilio'],
             data['colonia'], data['telefono'], data['observaciones'],
             json.dumps(data['respuestas'], ensure_ascii=False),
             json.dumps(data['entorno'], ensure_ascii=False),
             data['foto_path'], data['total_puntos']))
        con.commit()
...
st.set_page_config(page_title='Inspección de Fumigación', page_icon='🦟', layout='wide')
...
st.title('🦟 Inspección de Fumigación')
st.caption('Cuestionario de campo basado en el archivo proporcionado. Los puntajes se dejan configurables porque el Excel no contiene los valores numéricos de cada respuesta.')
...
st.markdown('<div class="section-title">EVIDENCIA FOTOGRÁFICA</div>', unsafe_allow_html=True)
photo = st.camera_input('Tomar fotografía de la vivienda / condición encontrada')
...
if st.button('💾 Guardar inspección', type='primary', use_container_width=True):
    ...
    if photo is not None:
        ...
        photo_path = str(UPLOAD_DIR / f'{folio or "sin_folio"}_{stamp}.jpg')
        Path(photo_path).write_bytes(photo.getvalue())
...
with st.expander('📋 Registros guardados'):
    with sqlite3.connect(DB_PATH) as con:
        df = pd.read_sql_query('SELECT id, created_at, folio, inspector, domicilio, colonia, total_puntos FROM inspecciones ORDER BY id DESC', con)
    if df.empty:
        st.caption('Todavía no hay inspecciones guardadas.')
    else:
